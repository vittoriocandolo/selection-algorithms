...
# linear plot
qp_lin <- ggplot(data = qp_avg_std, aes(x = n, y = avg, color = a)) +
  geom_point() +
  geom_line() +
...
ggsave("./qp_lin.png", width = 7.5)

# 2x log plot
qp_log <- ggplot(data = qp_avg_std, aes(x = n, y = avg, color = a)) +
  geom_point() +
  geom_line() +
  geom_smooth(method = "loess", linewidth = 0.25) +
